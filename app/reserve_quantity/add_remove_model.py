...
class AddRemoveReserve:
    ...
    def add_reserve_quantity(system_code, storage_id, count, customer_type, order_number):
        with MongoConnection() as client:
            ...
            if quantity_count > 0:
                product = client.product.find_one({"system_code": system_code},
                                                  {"_id": False, "warehouse_details": True})
                for cusrsor, storage_dict in product['warehouse_details'].get(customer_type)['storages'].items():
...
AddRemoveReserve.remove_reserve_quantity("2000010010001001001001001", "1", 2, "B2B", 1)
